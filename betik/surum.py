...
Kullanım: surum_kontrol(klasor) -> {"durum": guncel|eski|bozuk|yok, "metin": ..., "esitleme": ..., "isleme": ...}
...
def surum_kontrol(klasor=None, bugun=None, esik=ESIK_IS_GUNU):
    ...
    if os.path.exists(sg):
        for satir in open(sg, encoding="utf-8"):
            ...
            if os.path.exists(yol) and hashlib.sha256(open(yol, "rb").read()).hexdigest() != parca[0]:
                bozuk.append(ad)
    else:
