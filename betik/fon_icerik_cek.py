...
def sapma_sebepleri_yukle(veri):
    """veri/sapma_sebepleri.json: fon -> {sebep, kanit, tarih}. Listeye yalnizca incelenmis ve sebebi adlandirilmis fon girer."""
    yol = os.path.join(veri, "sapma_sebepleri.json")
    if os.path.exists(yol):
        SAPMA_SEBEPLERI.update({k: v for k, v in json.load(open(yol, encoding="utf-8")).items() if not k.startswith("_")})
...
AILE_ETIKET = {"hisse": "HİSSE SENEDİ", "yabanci_hisse": "YABANCI HİSSE SENEDİ", "dt": "DEVLET TAHVİLİ", "hb": "HAZİNE BONOSU", "ost": "ÖZEL SEKTÖR TAHVİLİ",
               "fb": "FİNANSMAN BONOSU", "kira": "KİRA SERTİFİKASI", "tpp": "TAKASBANK PARA PİYASASI", "repo": "TERS REPO", "maden": "KIYMETLİ MADEN",
               "byf": "BORSA YATIRIM FONU", "ybyf": "YABANCI BORSA YATIRIM FONU", "yf": "YATIRIM FONU", "katilim": "KATILIM HESABI", "mevduat": "VADELİ MEVDUAT",
               "teminat": "NAKİT TEMİNAT", "taahhut": "VADELİ İŞLEM TAAHHÜT", "dis_borc": "EUROBOND", "diger": "DİĞER"}
KOD_ETIKET = {"vdm": "VARLIĞA DAYALI MENKUL KIYMET"}


def tefas_tamamla(kayit, tefas_son):
    """Raporu eksik kalem tasiyan fon (Ziraat VDMK, 09.09.2026 karari): satirlarin toplami 100'un altinda kaliyor ve acik
    TEFAS'in tek bir kalemine 0,05 icinde esitse o kalem tek satirla eklenir; tutar TEFAS yuzdesi x FPD, ihracci bilinmiyor,
    kaynak tefas_kalan. Uydurma degil olcumdur: TEFAS fonun o kadar tuttugunu soyler, kirilimi bilinmez. Liste 100'e toplanir
    ama listeTam=false yazilir. Donus: (eklenen satirlar, aciklama)."""
    if not kayit or not tefas_son or not kayit[0].get("tlTaban"):
        return [], ""
    fpd = kayit[0]["tlTaban"]
    eksik = 100.0 - sum(k["agirlik"] for k in kayit)
    if eksik <= SAPMA_SERBEST:
        return [], ""
    k_aile, t_aile = katla(defaultdict(float, satir_aileleri(kayit)), tefas_aile(tefas_son))
    aday = []
    for kod, v in tefas_son.items():
        if kod in ("tarih", "fonKodu") or not v:
            continue
        v = float(v)
        if v <= SAPMA_SERBEST or abs(v - eksik) > SAPMA_SERBEST:
            continue
        aile = next((a for a, kodlar in AILE.items() if kod in kodlar), None)
        grup = UST_GRUP.get(aile, aile)
        if aile and t_aile.get(grup, 0.0) - k_aile.get(grup, 0.0) >= v - SAPMA_SERBEST:   # acik gercekten o ailede
            aday.append((kod, v, aile))
    if len(aday) != 1:
        return [], ""
    kod, v, aile = aday[0]
    etiket = KOD_ETIKET.get(kod, AILE_ETIKET.get(aile, aile.upper()))
    satir = dict(sayfa=0, ad="bilinmiyor", isin="", isinSayi=0, nominal=None, rayic=round(v * fpd / 100.0, 2), agirlik=v,
                 tur=f"{etiket} (TEFAS {kod} kalan)", kod="", ihracciHam="", ihracciSatir=0, kimlik="tefas_kalan", kaynak="tefas_kalan", tlTaban=fpd)
    return [satir], f"{kod} {v:.2f} puan tefas_kalan"
...
def duzen(t1, pdf_bayt=None):
    ...
    if "1- FONU TANITICI" in n and "FONUN ADI" in n:
        return "fonbul"
    if "AYLIK RAPORU" in n and "FON PORTFOY DEGERI TABLOSU" in n and "TEDAVULDEKI PAY SAYISI" in n:
        return "ziraat"
    if pdf_bayt:
...
def _sarilan_atama(R, anal, baslik, tablo_bas, tol, etiket_satir=()):
    """Ana satir olmayan satirlar (sarilan ad, ihracci devami, ISIN) ait olduklari kiymet satirina atanir.
    M59 (15 Eylul 2026): sarilan satir her zaman USTUNDEKI ana satira aittir (ad asagi dogru sarar). Onceki kural en yakin ana
    satira atiyordu; satir araligi dar raporlarda (Tera) devam satiri alttaki kiymete yapisiyor, kiymetAdiHam bir satir kayiyordu
    ("VE TİCARET A.Ş. ASELS ASELSAN ..."). Ustunde ana satir yoksa (sayfa basi) atanmaz; grup etiketi satirlari ad degildir."""
    atama = defaultdict(list)
    ana_sirali = sorted(anal)
    for i, (t, r) in enumerate(R):
        if i in anal or i in baslik or i <= tablo_bas or i in etiket_satir:
            continue
        metin = " ".join(w["text"] for w in r)
        if re.search(r"-\)|TOPLAMI|GÖRE\)", metin):
            continue
        ust = [a for a in ana_sirali if R[a][0] <= t]
        if not ust:
            continue
        j = ust[-1]
        if t - R[j][0] <= tol:
...
def satir_turleri(kayit):
    """M59: negatif nominalli satir ayni kiymetin pozitif satiriyla NET okunur. Kanit: Tera'nin bir hisse fonunun 2 Eylul raporunda ALKLC 2.750.000 ve
    -1.400.000 nominal, 9 Eylul raporunda 1.350.000 (takas bekleyen satis; varsayim, rapor bolum adi tasimiyor). Tuketiciler
    (denetim.sinama_bakis, icerik_olcu.anahtar) agirliklari zaten toplayarak netler. Donus, satir sirasiyla:
    'pozisyon' | 'negatif_eslesen' (ayni ISIN/kodun pozitif satiri var) | 'negatif_tek' (pozitif esi yok: acik pozisyon ya da borc)."""
    def anahtar(k):
        return k.get("isin") or k.get("kod") or k.get("ad")
    pozitif = {anahtar(k) for k in kayit if (k.get("agirlik") or 0) > 0}
...
def tefas_gun_esle(rows, f, ay, yayim, kayit, gruplar, evren, ay_ici=False):
    ...
    try:
        bit = datetime.strptime(str(yayim)[:10], "%d.%m.%Y").date().isoformat()
    except ValueError:
...
def yeniden_islenmeli(d, x, rap_ay, surum=None):
    """Kuyruk karari (M59): elde olan liste en yeni raporun kendisiyse atlanir. Ayni ay icin YENIDEN yayimlanan rapor
    (disclosureIndex farkli; Tera'nin bir fonu, 2 ve 9 Eylul) yeniden islenir; eski surumle uretilen liste de yeniden islenir."""
    surum = surum or AYRISTIRICI_SURUM
    if not (d.get("son") and d["son"] >= rap_ay and d.get("surum", 1) >= surum):
        return True
    return d.get("bildirim") != x.get("disclosureIndex")
        ...
        for pi, pg in enumerate(pdf.pages, start=1):
            ...
            for i, (t, r) in enumerate(R):
                ...
                if bekleyen and r[0]["x0"] < 60 and not re.search(r"\d", metin) and len(metin) < 60:
                    adaylar.append(metin); etiket_satir.add(i)
            ...
            atama = _sarilan_atama(R, anal, baslik, tablo_bas, tol, etiket_satir)
        ...
        for pi, pg in enumerate(pdf.pages, start=1):
            ...
            for t, r in R:
                ...
                if ilk == "Ana" and kal:
                    ...
                    if deg and ust:
                        gruplar.append(dict(sayfa=pi, tur=ust, anahtar=ust.split(".")[0], rayic=sayi(deg[-1]["text"]), agirlik=None, yaprak=False, icTutarlilik=True))  # yalnizca rapor ici tutarlilik (bolum harfi duzeyinde); 2. sarta girmez
                    continue
                ...
                if len(yuz) >= 2 and deg:
                    ...
                    kayit.append(dict(sayfa=pi, ad=" ".join(ad_tok), isin=isinler[0] if isinler else "", isinSayi=len(set(isinler)),
                                      ...
                                      tur=f"{ust} {alt}".strip(), anahtar=ust.split(".")[0], kod=ilk, ihracciHam=" ".join(ad_tok), ihracciSatir=1 if ad_tok else 0))
                    continue
        ...
        for pi, R in enumerate(sayfalar, start=1):
            ...
            for t, r in R:
                ...
                if ilk.startswith("TOPLAM"):
                    ...
                    gruplar.append(dict(sayfa=pi, tur=tur, anahtar=tur[:2], rayic=sayi(deg[-1]["text"]) if deg else None, agirlik=None, yaprak=True, icTutarlilik=True)); continue
                deg = [w for w in r if _sayi_mi(w["text"]) and kal["rayic"] - 45 <= w["x1"] <= kal["rayic"] + 6]
                ...
                if pct and deg and (r[0]["x0"] < kal["ihracci_x0"] or kodsuz_olur):
                    ...
                    if kod_i > 0:
                        ilk = sol[kod_i]          # basligin sarkan kelimeleri ("SENETLERİ") atilir, bolum turu degismez
...
def _sayi_tr(s):
    """Ziraat duzeni yalnizca Turk bicimi kullanir: '820.000.000' = 820000000, '912.693.219,27', '0,971791'.
    Genel sayi() nokta gruplarini Amerikan ondaligi sanabilir (90.079); bu duzende nokta daima binliktir."""
    return float(s.strip().replace(".", "").replace(",", "."))


def _ziraat_kalibre(r):
    m = {w["text"]: w for w in r}
    if "İhraçcı" in m and "Nominal" in m and "Rayiç" in m and "Oran" in m:
        k = dict(ihracci_x0=m["İhraçcı"]["x0"], nominal_x0=m["Nominal"]["x0"], rayic_x0=m["Rayiç"]["x0"], oran_x0=m["Oran"]["x0"])
        if "Birim" in m:
            k["fiyat_x0"] = m["Birim"]["x0"]
        return k
    return None


def ziraat_kiymetler(pdf_bayt):
    """Ziraat Portfoy duzeni ('<KOD>-... Agustos-2026 AYLIK RAPORU'): bolum basligi ile sutun basligi ayni satirda
    ('B - DEVLET TAHVILI ve HAZINE BONOLARI | Ihracci | Nominal | Rayic Deger | Oran (%) | Birim Alis Fiyati');
    hisse bolumunde alt basliklar ('1. Banka Istirak', '3. Yabanci Hisse Senetleri'). Satir: vade/sira, ISIN ya da kod
    (3AyaKadarVD-ZTB-TRY katilim hesabi), ihracci, nominal, rayic, oran (6 ondalik), birim alis fiyati (guncel degil, okunmaz).
    Grup toplami yok; FPD '4 - TOPLAM DEGERI TABLOSU' altindaki 'A. FON PORTFOY DEGERI'. Agirlik = rayic / FPD (TL tabani)."""
    kayit = []
    tur, alt, kal, fpd, bitti = "", "", None, None, False
    with pdfplumber.open(io.BytesIO(pdf_bayt)) as pdf:
        for pi, pg in enumerate(pdf.pages, start=1):
            R = _satirlar(pg); pg.flush_cache()
            for t, r in R:
                metin = " ".join(w["text"] for w in r)
                if re.match(r"^4 - TOPLAM", metin):
                    bitti = True
                if bitti:
                    if norm(metin).startswith("A. FON PORTFOY DEGERI"):
                        sayilar = [w["text"] for w in r if _sayi_mi(w["text"])]
                        if sayilar:
                            fpd = _sayi_tr(sayilar[0])
                    continue
                k2 = _ziraat_kalibre(r)
                if k2:
                    kal = k2
                    bas = re.sub(r"\s*İhraçcı.*$", "", metin)
                    tur = re.sub(r"^[A-ZÇĞİÖŞÜ]{1,2} - ", "", bas).strip(); alt = ""
                    continue
                if not kal:
                    continue
                if re.match(r"^\d\. ", metin) and len(metin) < 50 and not re.search(r"\d[.,]\d{2}", metin):
                    alt = metin; continue
                say = [w for w in r if _sayi_mi(w["text"])]
                ray = [w for w in say if kal["rayic_x0"] - 25 <= w["x0"] < kal["oran_x0"] - 15]
                oran = [w for w in say if kal["oran_x0"] - 15 <= w["x0"] < kal["oran_x0"] + 40]
                if not (ray and oran and r[0]["x0"] < kal["ihracci_x0"]):
                    continue
                nom = [w for w in say if kal["nominal_x0"] - 40 <= w["x0"] < kal["rayic_x0"] - 25]
                sol = [w["text"] for w in r if w["x0"] < kal["ihracci_x0"] - 5 and not TARIH_RE.match(w["text"]) and not re.match(r"^\d{1,3}$", w["text"])]
                isinler = [w["text"] for w in r if ISIN_RE.match(w["text"])]
                ih = [w["text"] for w in r if kal["ihracci_x0"] - 5 <= w["x0"] < kal["nominal_x0"] - 40 and not _sayi_mi(w["text"])]
...
AYRISTIRICILAR["ziraat"] = ziraat_kiymetler
...
def rapor_isle(f, x, kunye, kd, rows, evren, hedef):
    ...
    toplam_tab = toplam_tablosu(pdf) if d in ("standart", "standart-genis") else {}   # 78 numaralı not, bölüm 4: tablosuz düzende kuyruk kaydı {"olculemez": düzen} taşır, kalıcı ölçülemez, yeniden denenmez
    rapor_basligi = ray
    ...
    bilgi = dict(ray=ray, duzen=d, gun=gun, gunEsleme=eslesme, portfoyGunu=portfoy_gunu, gunOy=(f"{oy}/{oy_n}" if oy_n else ""), raporBasligi=rapor_basligi, sapma=sp, sapmaSebebi=sapma_sebebi,
                 toplamTablosu={k: toplam_tab.get(k) for k in ("fpd", "hazir", "alacak", "diger", "borc", "ihtiyat", "nav", "navSayfa1", "paySayisi", "kaldirac", "fpdNav")} if toplam_tab else ({"olculemez": d} if d not in ("standart", "standart-genis") else {}), tefasToplam=tefas_sinif_toplami(tefas_son), raporIci=rapor_ici, listeTam=liste_tam, eksikKalem=eksik_kalem, satir=len(kayit),
    ...
    if ok:
        ...
        turler = satir_turleri(kayit)                       # M59
        for k, satir_turu in zip(kayit, turler):
...
def kuyruk_turu(kunye, veri, arsiv, kurucu_filtre=None, fon_filtre=None):
    ...
    for f, d in list(ky.items()):            # eski kova adlarini ve 'ozel fon' varsayimini temizle
        if d.get("durum") == "hata" and d.get("sebep") == "ayrıştırma hatası: ":   # butce bitince yanlis yazilan kayitlar (08.09.2026)
            ky[f] = {k: v for k, v in d.items() if k in ("son", "surum", "sapma", "tefasGun", "satir", "bildirim", "yayim", "gunEsleme", "portfoyGunu", "gunOy", "raporBasligi", "toplamTablosu", "duzen")}
    ...
    try:
        ...
        def _fon_isle(f, x, d, rap_ay):
            ...
            if durum == "yayimlandi":
                ...
                ky[f] = dict(**{k: v for k, v in d.items() if k == "son"}, durum=durum, sebep=sebep, ay=ray, tarih=bit, bildirim=x.get("disclosureIndex"), yayim=x.get("publishDate"))
                kova[durum].append(f)
                if durum == "hata":
    ...
    for f in kunye:
        d = ky.get(f, {})
        if not d.get("durum"):
            ky[f] = dict(**{k: v for k, v in d.items() if k in ("son", "surum", "sapma", "tefasGun", "satir", "bildirim", "yayim", "gunEsleme", "portfoyGunu", "gunOy", "raporBasligi", "toplamTablosu", "duzen")}, durum="kuyrukta", sebep="sırası gelmedi", tarih=bit)
    json_yaz(ky_yol, ky)
...
def main():
    ...
    if a.pdf:
        ...
        kunye = {}; byf_yukle(a.veri); sapma_sebepleri_yukle(a.veri)
        for yol in a.pdf:
            ...
            ek, eksik_k = tefas_tamamla(kayit, son)
            if ek:
                kayit = kayit + ek
                ok, sebep, sp = kapi(kayit, gruplar, son, evren); ok, sebep, ssb = sapma_karari(fon, ok, sebep, sp, son)
            ...
            print(f"=== {fon} {d} {ray}: satır {len(kayit)}, yaprak grup {sum(1 for g in gruplar if g['yaprak'])}, ISIN'siz {sum(1 for k in kayit if not k['isin'])}, toplam "
                  f"{sum(k['agirlik'] for k in kayit):.2f}, satır içi {sh}/{ss}, rapor içi {ric}, {('eksik ' + eksik_k + ', ') if eksik_k else ''}TEFAS {gun} sapma {sp}{' ' + ssb if ssb else ''} | {'GEÇTİ' if ok else 'KALDI: ' + sebep}")
            if a.goster:
